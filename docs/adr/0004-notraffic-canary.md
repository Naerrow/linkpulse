...
- 상태: 채택 (2026-07-10) — **단, A-5 근본원인 절은 초안(플레이스홀더)이며 GameDay 드릴 실측 반영 후 확정**([`plan.md`](../plans/0006-p4-notraffic-canary/plan.md) step 8).
...
> **1차 출처 규율**: 위 "us-east-1 발행"·"지표 존재"·`request_interval` 실동작은 문서 근거에 더해 **사람이 `aws cloudwatch list-metrics --region us-east-1 --namespace AWS/Route53`로 실측 확정**한다(plan step 7). 미실측 단정이 A-5를 만든 재발 방지 원칙([[infra-plan-review-and-first-source]]).
...
## A-5 규명 — 근본원인 (초안: 플레이스홀더 + 증거수집 체크리스트 · 단정 금지)

> `alb-no-healthy-hosts`가 S2 2회 모두 MTTD 신호로 실패한 **근본원인은 아직 실측으로 확정하지 않았다.** 미실측 단정이 애초에 A-5를 만든 실수이므로(회고 §근본원인) 여기서 단정하지 않는다. 아래 가설과 증거 체크리스트를 두고, **step 8에서 GameDay 드릴 실측(또는 gameday-01 evidence)으로 이 절을 확정 서술로 갱신**한다.

**유력 가설(미확정):** desired=0 완전 다운 시 타깃이 전부 deregister되면서 `HealthyHostCount` 지표가 **missing이 됐고**, `treat_missing_data=breaching`이 발화하기까지의 타이밍(또는 지표 발행 공백)이 예측(4~6분)과 어긋났다. missing이 아니라 0이 계속 발행됐다면 다른 원인(평가 창·집계 지연)을 봐야 한다.

**증거수집 체크리스트(step 7 드릴에서 캡처 → step 8에서 반영):**

- [ ] desired=0 구간의 `HealthyHostCount` datapoint를 `aws cloudwatch get-metric-statistics`로 캡처 → **missing이었나 0이었나** 판별.
- [ ] `alb-no-healthy-hosts` 발화 시각(`describe-alarm-history`) 기록 → MTTD 재측정.
- [ ] 같은 구간 `canary_down`/`alb-elb-5xx` 발화 시각과 대조.
- [ ] 실측이 missing/0을 명확히 못 가리면 "문서화된 AWS 동작과 정합하며 실측(9분48초/무발화)이 이를 시사한다"는 **근거-인용 완화형**으로 확정.
...
2. **무트래픽 1차 방어선은 canary가 맡는다.** MTTD 역할 분리:
   - `alb-elb-5xx`(canary 트래픽 기반, 회고상 ~3–5분) = **빠른 1차**.
   - `canary_down`(≈flip 90s + 알람 3분 ≈ 4.5분+) = **결정론적 백스톱**.
   - 둘 다 `alb-no-healthy-hosts`(9분48초/무발화)보다 빠르다.
