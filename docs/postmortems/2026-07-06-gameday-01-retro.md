...
| P6 | `alb-no-healthy-hosts`가 S2 MTTD 담당(4~6분) | 2회 모두 예측과 다름(9분48초/복구후, 또는 미발화) | **빗나감** | 이번(트래픽 있는) 드릴에선 MTTD를 `alb-elb-5xx`에 내줌 — 다만 이 알람의 원래 역할인 **무트래픽 다운 감지의 적합성은 미규명**(A-5). "MTTD 신호로 부적합"이라 단정하지 않는다 |
...
| A-2 | EventBridge `SERVICE_DEPLOYMENT_FAILED`→SNS→Slack 배포 실패 통지 | **백로그(P4 초반, 우선순위 높음)** | 이번 드릴은 오히려 A-2의 **가치를 실증**했다 — 1차 세트에서 사람이 Actions run 확인을 놓친 사각지대를, 배포 실패를 Slack으로 밀어주는 이 통지가 정확히 메운다. 다만 P3-2를 얇게 유지하는 범위 원칙상 **P3 완료 필수는 아니고** 무인 배포 실패 감지 강화 목적의 P4/하드닝으로 이관 |
...
| A-5 | `alb-no-healthy-hosts` 지연/미발화 원인 규명 | **백로그(P4, 우선순위 높음)** | 단순 조사가 아니라 **"무트래픽 다운 방어선"의 설계 검증 과제**. 이 알람의 존재 이유가 "트래픽 없어도 다운 감지"(breaching 설계)인데 실측상 MTTD 신호로는 2회 다 실패(미발화/복구후). 튜닝·교체 여부까지 P4 관측성 하드닝에서 판단. **A-8은 이 규명 전까지의 운영 우회임** |
...
| A-8 | runbook §0-2의 "S2 1순위 확인 알람"을 `no-healthy-hosts`가 아니라 `alb-elb-5xx`로 재정의 (**caveat**: 트래픽/synthetic curl 있는 다운 한정) | **반영 완료**(runbook §0-2·§2) | 2회 실측 모두 이 알람이 실제 MTTD 담당(3분37초·4분54초) — 회고 핵심 발견. **단 무트래픽 다운에선 5xx 자체가 안 생겨 이 알람이 안 뜰 수 있으므로 "1순위"는 트래픽 있는 다운으로 한정**, 무트래픽 방어선은 A-5로 남긴다(A-8=임시 운영 우회). caveat까지 §0-2·§1·§2에 함께 반영 |
...
| A-10 | 상시 synthetic canary(주기적 healthz/기능 요청) 도입 검토 | **백로그(P4 관측성)** | 이번 MTTD 3~5분은 C-1 폴링(=synthetic 트래픽)이 있었기에 나온 값 — 상시 canary를 두면 **무트래픽 다운도 `alb-elb-5xx`로 감지**되어 A-5(no-healthy-hosts 규명)의 유력한 대안이 된다. A-5와 함께 P4에서 저울질 |

**요약**: 반영 완료 5(A-4·A-6·A-7·A-8·A-9 — 문서 working tree 수정 끝, 커밋만 사람) + 통합 1(A-3→A-6) + 백로그 3(A-2·A-5·A-10) + 기각 1(A-1) = **전수 10건 종결**.
